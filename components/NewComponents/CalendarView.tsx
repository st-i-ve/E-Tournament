...
import { Calendar, Clock, MapPin, Home, Users, ChevronLeft, ChevronRight, CalendarDays, List } from 'lucide-react';
    ...
    return (
      <div className="bg-gray-900/40 rounded-2xl p-12 border border-gray-800/50">
        ...
        <div className="grid grid-cols-7 gap-6 mb-8">
          {weekDays.map(day => (
            <div key={day} className="p-6 text-center text-xl font-semibold text-gray-300 bg-gray-800/30 rounded-lg">
              {day}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-6">
          {days.map((day, index) => {
            ...
            const homeMatches = matches.filter(m => m.homeTeam === currentUser.teamName);
            const awayMatches = matches.filter(m => m.awayTeam === currentUser.teamName);
            ...
            return (
              <div 
                ...
                className={`p-6 h-40 border-2 border-gray-700 rounded-xl cursor-pointer transition-all hover:bg-gray-800/70 hover:border-gray-600 ${
                  isToday ? 'bg-green-500/15 border-green-500/50' : 'bg-gray-800/20'
                } ${isSelected ? 'bg-blue-500/25 border-blue-500/60' : ''}`}
                onClick={() => setSelectedDate(isSelected ? null : day)}
              >
                <div className={`text-2xl font-bold mb-4 ${isToday ? 'text-green-400' : 'text-gray-200'}`}>
                  {day.getDate()}
                </div>
                ...
                {matches.length > 0 && (
                  <div className="flex items-center justify-center gap-3">
                    {homeMatches.length > 0 && (
                      <div className="w-6 h-6 bg-green-500 rounded-full flex items-center justify-center" title="Home match">
                        <span className="text-xs font-bold text-white">{homeMatches.length}</span>
                      </div>
                    )}
                    {awayMatches.length > 0 && (
                      <div className="w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center" title="Away match">
                        <span className="text-xs font-bold text-white">{awayMatches.length}</span>
                      </div>
                    )}
                  </div>
                )}
