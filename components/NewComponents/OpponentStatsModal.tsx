...
import { Trophy, Target, Shield, TrendingUp, Users, X } from 'lucide-react';
...
const OpponentStatsModal = ({ opponent, isOpen, onClose }: OpponentStatsModalProps) => {
  ...
  const headToHeadMatches = mockMatches.filter(match => 
    (match.homeTeam === currentUser.teamName && match.awayTeam === opponent) ||
  ...
  const getFormIcon = (result: string) => {
    ...
    return (
      <div className={`flex items-center justify-center h-6 w-6 rounded-sm ${styles[result as keyof typeof styles]}`}>
        ...
        <DialogHeader>
          <DialogTitle className="text-white flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-gray-400">{opponentUser?.username || 'Unknown Player'}</div>
            </div>
          </DialogTitle>
        </DialogHeader>
            ...
            {h2hStats.totalMatches > 0 ? (
              <div className="bg-gray-800/50 rounded-lg p-4">
                <div className="grid grid-cols-3 gap-4 text-center mb-3">
                  <div>
                    <div className="text-lg font-bold text-green-400">{h2hStats.wins}</div>
                    <div className="text-xs text-gray-400">Wins</div>
                  </div>
                  <div>
                    <div className="text-lg font-bold text-yellow-400">{h2hStats.draws}</div>
                    <div className="text-xs text-gray-400">Draws</div>
                  </div>
                  <div>
                    <div className="text-lg font-bold text-red-400">{h2hStats.losses}</div>
            ...
            <div className="bg-gray-800/50 rounded-lg p-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-medium">#{seasonStats.position}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Points:</span>
                  <span className="text-white font-medium">{seasonStats.points}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Matches:</span>
                  <span className="text-white font-medium">{seasonStats.matchesPlayed}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Goals:</span>
                  <span className="text-white font-medium">{seasonStats.goalsFor}:{seasonStats.goalsAgainst}</span>
                </div>
              </div>
            </div>
          </div>
