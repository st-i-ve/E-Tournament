...
import { Trophy, Users, Crown, Calendar, Star, Medal } from 'lucide-react';
...
const TournamentCard = ({ tournament, isAdmin = false }: TournamentCardProps) => {
      ...
      <CardContent className="p-4 relative">
        ...
        <div className="flex items-start justify-between mb-3 relative z-10">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mb-3">
              <Badge 
                variant="outline" 
                className={`text-xs border-0 ${tournament.type === 'league' 
              ...
              <Badge variant="outline" className="border-0 bg-green-500/20 text-green-400 text-xs">
                ACTIVE
              </Badge>
            </div>
          </div>
          ...
          {tournament.userPosition && (
            <div className={`px-3 py-1.5 rounded-full text-xs font-bold text-white ${getPositionColor(tournament.userPosition)} flex items-center gap-1 shadow-lg`}>
        ...
        <div className="flex items-center justify-between text-xs">
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 text-gray-400">
              ...
              <span className="font-medium">{tournament.totalParticipants}/{tournament.maxParticipants}</span>
            </div>
          ...
          <div className="text-right">
            <div className="text-green-400 font-semibold text-xs">
              {Math.round((tournament.totalParticipants / tournament.maxParticipants) * 100)}% full
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 w-full h-1 bg-gray-700 rounded-full overflow-hidden">
          <div 
            ...
            style={{ width: `${(tournament.totalParticipants / tournament.maxParticipants) * 100}%` }}
          />
        </div>
      </CardContent>
    </Card>
