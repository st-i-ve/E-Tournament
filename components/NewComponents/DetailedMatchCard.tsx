...
const DetailedMatchCard = ({ match, userTeam, onViewDetails }: DetailedMatchCardProps) => {
  ...
  return (
    <div className={`${getResultColor()} rounded-xl p-3 transition-all duration-200 border border-gray-800/50 hover:border-gray-700/50 cursor-pointer`}
         onClick={() => onViewDetails(match)}>
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 text-xs">
              <span className={`truncate ${match.team_home.name === userTeam ? 'font-semibold text-green-400' : 'text-gray-300'}`}>
                {match.team_home.name}
              </span>
              <span className="text-gray-500">vs</span>
              <span className={`truncate ${match.team_away.name === userTeam ? 'font-semibold text-green-400' : 'text-gray-300'}`}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <Badge variant="outline" className={`text-xs border-0 ${result.color} flex items-center gap-1 px-1 py-0.5`}>
